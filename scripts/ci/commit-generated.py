...
def main() -> int:
    ...
    result = subprocess.run(
        ["gh", "api", "graphql", "-f", f"query={MUTATION}",
         "--input", "-", "--jq", ".data.createCommitOnBranch.commit.url"],
        input=json.dumps({"query": MUTATION, "variables": variables}),
        capture_output=True, text=True,
    )
    if result.returncode != 0:
        stderr = result.stderr
        # Losing the compare-and-swap is the expected outcome of a long
        # regeneration racing a push, not a failure: the next run starts from
        # the head that won.
        if "expected head" in stderr.lower() or "not match" in stderr.lower():
            print(f"::notice::{branch} moved while regenerating; "
                  "leaving it to the next run")
            return 0
        print(f"::error::createCommitOnBranch failed: {stderr}", file=sys.stderr)
        return 1

    print(f"committed {len(paths)} path(s): {result.stdout.strip()}")
    return 0
